document.addEventListener("DOMContentLoaded", () => {
  ...
  const subjectName = "MA Education";
  ...
  function initializeQuiz() {
    ...
    userAnswers = Array(questions.length)
      .fill(null)
      .map(() => ({ selectedOption: null, isCorrect: null })); // Initialize with null for both
  ...
  function displayQuestion() {
    ...
    if (previousAnswerState && previousAnswerState.selectedOption) {
      const selectedOptionElement = optionsList.querySelector(
        `[data-option="${previousAnswerState.selectedOption}"]`
      );
    ...
    if (previousAnswerState && previousAnswerState.isCorrect !== null) {
      ...
      feedbackMessage.textContent = previousAnswerState.isCorrect
        ? "Correct!"
        : "Incorrect!";
      feedbackMessage.classList.add(
        previousAnswerState.isCorrect ? "text-green-700" : "text-red-700"
      );
    } else {
      ...
      optionElement.addEventListener("click", () => {
        ...
        if (userAnswers[currentQuestionIndex].isCorrect === null) {
          optionsList
            ...
            .forEach((opt) => opt.classList.remove("option-selected"));
  ...
  function updateTimerDisplay() {
    ...
    timerSpan.textContent = `${String(hours).padStart(2, "0")}:${String(
      minutes
